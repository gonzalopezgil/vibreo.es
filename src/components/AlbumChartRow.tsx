...
function ChangeIndicator({ entry }: { entry: AlbumChartEntry }) {
  if (entry.entry_status === 'NEW_ENTRY' || (entry.previous_rank === 0 && entry.weeks_on_chart <= 1)) {
...
export function AlbumChartRow({
  ...
  const albumId = entry.uri.split(':').pop() || '';
  ...
  return (
    ...
    >
      ...
      <div className="min-w-0 flex-1">
        <p className="truncate text-sm font-semibold text-zinc-100">{entry.album_name}</p>
        <p className="truncate text-xs text-zinc-400">{entry.artist_names.split('|').join(', ')}</p>
      </div>

      <div className="shrink-0 text-right">
        <p className="text-xs text-zinc-500">
          {entry.weeks_on_chart} {entry.weeks_on_chart === 1 ? 'wk' : 'wks'}
          {entry.peak_rank > 0 && <span className="ml-1.5">· #{entry.peak_rank}</span>}
        </p>
      </div>
    </Link>
  );
